...
const { generateImage, createImagePrompt } = require('../services/imageService');
...
router.get('/recipe/:recipeId/image/:paragraphIndex', async (req, res, next) => {
    ...
    try {
        ...
        const storyData = await cache.getRecipeStory(recipeId);
        if (!storyData || !storyData.story) {
            return res.status(404).json({ message: 'No story found for this recipe.' });
        }
        ...
        if (parsedIndex >= paragraphs.length) {
            return res.status(404).json({ message: `Paragraph index ${parsedIndex} not found in story.` });
        }
        ...
        console.error(`Error generating image for recipe ${recipeId}, paragraph ${paragraphIndex}:`, error);
        next(error);
    }
});
