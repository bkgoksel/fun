...
async function appendToRecipeStory(
  recipeId,
  storyAddition,
  expirationInSeconds = 3600 * 24,
) {
...
async function setRecipeImageUrl(
  recipeId,
  paragraphIndex,
  imageUrl,
  expirationInSeconds = 3600 * 24 * 7,
) {
  const key = `recipe:${recipeId}:image:${paragraphIndex}`;
...
async function getAllRecipeImageUrls(recipeId) {
  try {
    ...
    console.error(
      `Error getting all recipe image URLs for recipe ${recipeId}:`,
      error,
    );
    return {};
  }
